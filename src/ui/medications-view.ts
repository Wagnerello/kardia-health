...
function formatarDosagem(dosagem?: string): string {
  if (!dosagem) return '';
  const d = dosagem.trim();
  if (/^\d+$/.test(d)) {
    return `${d} mg`;
  }
  return d;
...
function formatarStatusMedicacaoItem(m: Medication): { statusHtml: string; isConcluido: boolean } {
  if (m.tipo === 'Temporario' && m.data_fim) {
    const agora = new Date();
    if (agora > m.data_fim) {
      return {
        statusHtml: '<span class="medication-badge medication-badge-muted">Concluído</span>',
        isConcluido: true
      };
    }
    const diasFaltam = Math.ceil((m.data_fim.getTime() - agora.getTime()) / (1000 * 3600 * 24));
    return {
      statusHtml: `<span class="medication-badge medication-badge-info">Faltam ${diasFaltam} dia${diasFaltam === 1 ? '' : 's'}</span>`,
      isConcluido: false
    };
  }
  return { statusHtml: '', isConcluido: false };
}

function renderizarItemMedicacao(m: Medication): string {
  const { statusHtml, isConcluido } = formatarStatusMedicacaoItem(m);
  const dosagemFormatada = formatarDosagem(m.dosagem);
  const freqFormatada = formatarFrequencia(m.frequencia);
  const tipoLabel = m.tipo === 'Continuo' ? 'Uso Contínuo' : 'Uso Temporário';
  const tipoBadgeClass = m.tipo === 'Continuo' ? 'medication-badge-success' : 'medication-badge-info';

  return `
  <li class="medication-item ${isConcluido ? 'medication-item-concluido' : ''}" role="listitem">
    <div class="medication-item-info">
      <div class="medication-title">
        <span>${m.nome}</span>
        ${dosagemFormatada ? `<span class="medication-dosage-badge">${dosagemFormatada}</span>` : ''}
      </div>
      <div class="medication-details">
        <span>${freqFormatada}</span>
        <span style="opacity:0.4">•</span>
        <span class="medication-badge ${tipoBadgeClass}">${tipoLabel}</span>
        ${statusHtml}
      </div>
    </div>
    <div class="medication-actions">
      <button class="medication-action-btn medication-action-btn-edit" onclick="editarMedicationId('${m.id}')" title="Editar ${m.nome}" aria-label="Editar ${m.nome}">
      ...
      <button class="medication-action-btn medication-action-btn-delete" onclick="excluirMedicationId('${m.id}')" title="Excluir ${m.nome}" aria-label="Excluir ${m.nome}">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
      </button>
    </div>
  </li>`;
}
...
export const carregarMedications = async () => {
  ...
  try {
    ...
    if (state.medications.length === 0) {
      list.innerHTML = '<li style="color:var(--text-muted); font-size:14px; text-align:center; padding:20px 0; list-style:none;">Nenhuma medicação cadastrada.</li>';
      return;
    }
